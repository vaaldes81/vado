...
QBCore.Commands.Add('mfine', 'Fine A Person', {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Fine Amount'}}, false, function(source, args)
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('QBCore:Notify', source, 'Fine has been issued to citizen succesfully', 'success')
                    TriggerClientEvent('QBCore:Notify', billed.PlayerData.source, 'State Debt Recovery has automatically recovered the fines owed...')
                    TriggerEvent('qb-banking:society:server:DepositMoney', biller, amount , 'mechanic')
                else
